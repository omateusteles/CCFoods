    ...
    public partial class PedidosPage : ContentPage
    {
        ...
        private async void OnProduzirClick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    smsMessenger.SendSms(pedido.Cliente.Telefone, "Pedido " + pedido.PedidoId + " enviado para a produção. CCFoods, obrigado pela preferência");
                }
                else
                {
                    await DisplayAlert("Atenção", "O SMS não pôde ser enviado, mas o pedido foi enviado para a produção", "Ok");
                    SelecionarPedidosEmAberto();
                }
            }
        }
        ...
        private async void OnEntregarClick(object sender, EventArgs e)
        {
            ...
            {
                await DisplayAlert("Atenção", "Pedido precisa estar em produção", "Ok");
            }
            else
            {
                ...
                if (smsMessenger.CanSendSms)
                {
                    smsMessenger.SendSms(pedido.Cliente.Telefone, "Pedido " + pedido.PedidoId + " enviado para a entrega. CCFoods, obrigado pela preferência");
                }
                else
                {
                    await DisplayAlert("Atenção", "O SMS não pôde ser enviado, mas o pedido foi enviado para a entrega", "Ok");
                    SelecionarPedidosEmAberto();
                }
            }
        }
        ...
        private async void OnFecharClick(object sender, EventArgs e)
        {
            ...
            {
                await DisplayAlert("Atenção", "Pedido precisa estar em entrega", "Ok");
            }
            else
            {
                ...
                if (smsMessenger.CanSendSms)
                {
                    smsMessenger.SendSms(pedido.Cliente.Telefone, "Pedido " + pedido.PedidoId + " finalizado com sucesso. CCFoods, obrigado pela preferência");
                }
                else
                {
                    await DisplayAlert("Atenção", "O SMS não pôde ser enviado, mas o pedido foi finalizado com sucesso", "Ok");
                    SelecionarPedidosEmAberto();
                }
            }
        }
        ...
        private async void OnVerificarRotaClick(object sender, EventArgs e)
        {
            ...
            var endereco = pedido.Cliente.Endereco + " " + pedido.Cliente.Numero + " " + pedido.Cliente.Bairro + " " + pedido.Cliente.Cidade + " " + pedido.Cliente.Estado;
            switch (Device.RuntimePlatform)
            {
                case Device.iOS:
                    Device.OpenUri(new Uri(string.Format("http://maps.apple.com/?saddr ={ 0 } & daddr ={ 1}&dirflg = d", WebUtility.UrlEncode(origem), WebUtility.UrlEncode(endereco))));
                    break;
                case Device.Android:
