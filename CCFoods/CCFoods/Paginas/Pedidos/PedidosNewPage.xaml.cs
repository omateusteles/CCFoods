...
namespace Modulo1.Paginas.Pedidos
{
    ...
    public partial class PedidosNewPage : ContentPage
    {
        ...
        public async void OnRemoverClick(object sender, EventArgs e)
        {
            ...
            var item = mi.CommandParameter as ItemPedido;
            var itemCardapio = itemCardapioDAL.GetItemById(Convert.ToInt32(item.ItemCardapioId));

            var opcao = await DisplayAlert("Confirmação de exclusão", "Confirma excluir o item " + itemCardapio.Nome.ToUpper() + "?", "Sim", "Não");

            if (opcao)
            {
                // removendo o item da collection
